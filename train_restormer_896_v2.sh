...
#SBATCH --mem=64G
#SBATCH -t 24:00:00
#SBATCH -o train_restormer_896_v2_%j.out
#SBATCH -e train_restormer_896_v2_%j.err
#SBATCH -J rest_896

echo "=========================================="
echo "Restormer 896x896 - Proper Train/Val/Test"
echo "=========================================="
...
class HDRDataset(Dataset):
    def __init__(self, data_root, jsonl_path, split='train', crop_size=896, test_jsonl='test.jsonl'):
...
train_ds = HDRDataset('.', 'train.jsonl', 'train', 896)
val_ds = HDRDataset('.', 'train.jsonl', 'val', 896)
print(f'Train: {len(train_ds)}, Val: {len(val_ds)}')

train_loader = DataLoader(train_ds, batch_size=1, shuffle=True, num_workers=4, pin_memory=True)
val_loader = DataLoader(val_ds, batch_size=1, shuffle=False, num_workers=2)

# Optimized hyperparameters for 896:
# - Lower LR due to batch_size=1 (noisier gradients)
# - Higher grad_accum for effective batch size of 8
...
output_dir = Path('outputs_restormer_896_v2')
...
for epoch in range(total_epochs):
    ...
    for i, batch in enumerate(pbar):
        ...
        if i == 0 and epoch == 0:
            print(f'GPU memory during training: {torch.cuda.max_memory_allocated()/1e9:.2f} GB')
    ...
    print(f'Epoch {epoch+1}: Train={epoch_loss/len(train_loader):.4f}, Val={val_loss:.4f}, LR={lr:.2e}' + (' (best)' if is_best else ''))
