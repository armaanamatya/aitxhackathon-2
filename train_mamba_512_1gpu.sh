...
# =============================================================================
# MambaDiffusion 512x512 - Single GPU with Gradient Checkpointing
# =============================================================================

echo "=========================================="
echo "MambaDiffusion 512x512 - 1 GPU"
echo "=========================================="
...
echo ""
echo "Config: 512x512, batch=1, grad_accum=8, mixed precision"
echo ""

# Use the existing optimized training script with 512 resolution
python3 -c "
import sys
...
class HDRDataset(Dataset):
    def __init__(self, data_root, jsonl_path, split='train', crop_size=512):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and random.random() > 0.5:
            src = torch.flip(src, [2])
            tar = torch.flip(tar, [2])
...
model = mamba_diffusion_large().to(device)
...
train_ds = HDRDataset('.', 'train.jsonl', 'train', 512)
val_ds = HDRDataset('.', 'train.jsonl', 'val', 512)
...
output_dir = Path('outputs_mamba_512')
...
for epoch in range(100):
    ...
    print(f'Epoch {epoch+1}: Train={epoch_loss/len(train_loader):.4f}, Val={val_loss:.4f}' + (' (best)' if val_loss < best_val else ''))
    ...
    if (epoch + 1) % 20 == 0:
        torch.save({'epoch': epoch, 'model_state_dict': model.state_dict()}, output_dir / f'checkpoint_epoch_{epoch+1}.pt')
