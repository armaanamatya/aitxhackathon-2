#!/usr/bin/env python3
"""
DarkIR Training with Window-Aware Loss
=======================================
Combines DarkIR's FFT-based architecture with window-focused loss weighting.
"""
...
from window_aware_loss import WindowAwareLoss, get_window_aware_loss


class RealEstateDataset(Dataset):
    """Real estate dataset that returns input for window detection"""
...
def train_epoch(model, dataloader, optimizer, criterion, device):
    """Train for one epoch with window-aware loss"""
    model.train()
    total_loss = 0
    ...
    for src, tar in tqdm(dataloader, desc="Training", leave=False):
        ...
        # Window-aware loss uses input image for detection
        loss_dict = criterion(out, tar, input_img=src, return_components=True)
        ...
        total_window_weight += loss_dict['window_mask_mean'].item()
        count += 1

    return total_loss / count, total_window_weight / count
...
def main():
    parser = argparse.ArgumentParser(description="Train DarkIR with Window-Aware Loss")
    ...
    # Loss configuration
    parser.add_argument('--loss_config', type=str, default='default',
                        choices=['default', 'aggressive', 'light'],
                        help='Window-aware loss configuration')
    parser.add_argument('--window_weight', type=float, default=3.0,
                        help='Weight multiplier for window regions')
    ...
    print("=" * 70)
    print("DARKIR TRAINING WITH WINDOW-AWARE LOSS")
    print("=" * 70)
    print(f"Resolution: {args.resolution}")
    print(f"Window weight: {args.window_weight}x")
    ...
    # Create window-aware loss
    criterion = get_window_aware_loss(args.loss_config, device=args.device)
    print(f"Using WindowAwareLoss ({args.loss_config} config)")
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, window_weight = train_epoch(model, train_loader, optimizer, criterion, args.device)
        ...
        print(f"Epoch {epoch:3d}/{args.epochs}: "
              ...
              f"WinMask={window_weight:.3f}, LR={current_lr:.2e} "
