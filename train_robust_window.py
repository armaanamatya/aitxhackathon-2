#!/usr/bin/env python3
"""
Robust Window Recovery Training
===============================

Uses delta-aware loss that:
1. Identifies what NEEDS fixing (source ≠ target)
2. Preserves what's already good (source ≈ target)
3. Applies different treatment per region

This generalizes to:
- Super bright blown-out windows → aggressive fix
- Already good windows → preserve
- Varying contrast/luminance → adaptive per-region
"""
...
from training.delta_aware_loss import RobustWindowRecoveryLoss, create_robust_window_loss
...
class HDRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            # Random brightness/contrast augmentation (helps generalization)
            if torch.rand(1) > 0.7:
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='outputs_robust_window')
    ...
    print("=" * 70)
    print("ROBUST WINDOW RECOVERY TRAINING (Delta-Aware)")
    print("=" * 70)
    ...
    print()
    print("Key Features:")
    print("  - Delta-aware: fixes bad windows, preserves good ones")
    print("  - Direction loss: ensures correct transformation direction")
    print("  - Preservation loss: prevents overcorrection")
    print("  - Brightness augmentation: improves generalization")
    print()
    ...
    print(f"  Direction lambda: {criterion.delta_loss.lambda_direction}")
    print(f"  Preserve lambda: {criterion.delta_loss.lambda_preserve}")
    print()
