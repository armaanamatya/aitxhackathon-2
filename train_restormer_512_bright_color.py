#!/usr/bin/env python3
"""
Restormer 512 Training with Enhanced Bright Color Loss

Focus: Better matching of colors in bright regions (windows, sky, plants)

Loss Components:
- L1 (1.0): Primary pixel accuracy
- Window (0.3): Extra weight on bright regions
- BrightColor (0.5): Color accuracy in bright regions
  - Hue: Preserve exact color type (blue sky, green plants)
  - Chroma: Preserve colorfulness/saturation
  - RGB: Per-channel accuracy in bright regions
...
from training.bright_color_loss import EnhancedCombinedLoss
...
def main():
    parser = argparse.ArgumentParser(description='Restormer 512 with Enhanced Bright Color Loss')
    ...
    parser.add_argument('--output_dir', type=str, default='outputs_restormer_512_bright_color')
    ...
    parser.add_argument('--window_weight', type=float, default=0.3)
    parser.add_argument('--bright_color_weight', type=float, default=0.5)
    parser.add_argument('--brightness_threshold', type=float, default=0.4,
                        help='Threshold for bright region detection (lower = more regions)')
    ...
    print("=" * 80)
    print("RESTORMER 512 - ENHANCED BRIGHT COLOR LOSS")
    print("=" * 80)
    ...
    print(f"  Window: {args.window_weight}")
    print(f"  BrightColor: {args.bright_color_weight}")
    print(f"  Brightness Threshold: {args.brightness_threshold}")
    print()
    ...
    # Loss - Enhanced with bright color components
    criterion = EnhancedCombinedLoss(
        l1_weight=args.l1_weight,
        window_weight=args.window_weight,
        bright_color_weight=args.bright_color_weight,
        brightness_threshold=args.brightness_threshold,
    )
    print("Loss: L1 + Window + BrightColor (Hue + Chroma + RGB + Histogram)")
    print()
    ...
    if args.resume:
        ...
        print(f"Resumed from epoch {start_epoch}, best val L1: {best_val_l1:.4f}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(f"Epoch {epoch+1:3d}/{args.epochs} | "
              ...
              f"Win: {val_components.get('window', 0):.4f} | "
              f"BrightCol: {val_components.get('bright_color', 0):.4f} | "
              f"LR: {current_lr:.2e} | Time: {epoch_time:.1f}s{best_marker}")

        # Print color sub-components every 10 epochs
        if (epoch + 1) % 10 == 0:
            print(f"    Color breakdown: "
                  f"Hue={val_components.get('color_hue', 0):.4f} | "
                  f"Chroma={val_components.get('color_chroma', 0):.4f} | "
                  f"RGB={val_components.get('color_rgb', 0):.4f} | "
                  f"Hist={val_components.get('color_histogram', 0):.4f}")
