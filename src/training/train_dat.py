"""
DAT Training Script for Image Retouching
=========================================

Training script for Dual Aggregation Transformer (ICCV 2023).
Optimized for _src.jpg -> _tar.jpg matching.
...
# Import model
from dat import DAT, dat_small, dat_base, dat_large
...
class DATTrainer:
    ...
    def train(self):
        print(f"\nStarting DAT training from epoch {self.start_epoch}")
...
# =============================================================================
# DAT Configuration for ~26M params (similar to Restormer-base)
# =============================================================================

def dat_restormer_size() -> DAT:
    """
    DAT configured for good capacity while being memory efficient.
    Reduced from 6 layers to 4 layers to fit in memory.
    """
    return DAT(
        embed_dim=120,  # Reduced for memory
        depths=(6, 6, 6, 6),  # 4 layers instead of 6
        num_heads=(6, 6, 6, 6),
        window_size=8,
        mlp_ratio=2.0,
        qkv_bias=True,
...
def parse_args():
    parser = argparse.ArgumentParser(description='DAT Training')

    parser.add_argument('--data_root', type=str, default='.')
    parser.add_argument('--jsonl_path', type=str, default='train.jsonl')
    parser.add_argument('--output_dir', type=str, default='outputs_dat')
    parser.add_argument('--image_size', type=int, default=256)

    parser.add_argument('--model_size', type=str, default='restormer_size',
                        choices=['small', 'base', 'large', 'restormer_size'])
...
def main():
    ...
    print(f"Initializing DAT-{args.model_size}...")
    if args.model_size == 'small':
        ...
        model = dat_large()
    elif args.model_size == 'restormer_size':
