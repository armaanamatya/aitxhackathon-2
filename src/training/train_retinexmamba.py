"""
Training Script for RetinexMamba HDR Enhancement Model

Features:
- RetinexMamba architecture with illumination-guided enhancement
- Multiple loss functions: Charbonnier, SSIM, FFT, Perceptual, LPIPS, LAB
- Illumination consistency loss for Retinex decomposition
...
from src.training.retinexmamba import (
    retinexmamba_small,
    retinexmamba_base,
    retinexmamba_large,
    count_parameters,
...
class IlluminationConsistencyLoss(nn.Module):
    """
    Loss to ensure illumination decomposition is consistent.
    Encourages: image ≈ illumination × reflectance
    """
    def __init__(self):
        super().__init__()

    def forward(self, image, illumination, reflectance):
        """
        image: (B, 3, H, W) in [0, 1]
        illumination: (B, 1, H, W) in [0, 1]
        reflectance: (B, 3, H, W) in [0, 1]
        """
        reconstructed = illumination * reflectance
        return F.l1_loss(reconstructed, image)
...
class RetinexMambaTrainer:
    """
    Trainer for RetinexMamba HDR Enhancement.
    """
    ...
    def __init__(
        ...
        # Model
        print(f"Initializing RetinexMamba-{model_size}...")
        if model_size == "small":
            self.model = retinexmamba_small().to(self.device)
        elif model_size == "base":
            self.model = retinexmamba_base().to(self.device)
        elif model_size == "large":
            self.model = retinexmamba_large().to(self.device)
        else:
        ...
        if lambda_illum > 0:
            self.criterion_illum = IlluminationConsistencyLoss().to(self.device)
            print("Using Illumination Consistency loss")
        else:
            self.criterion_illum = None
    ...
    def train_epoch(self, epoch: int) -> dict:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with autocast(enabled=self.use_amp):
                ...
                if self.criterion_illum is not None:
                    illum, reflect = self.model.retinex((source + 1) / 2)
                    source_01 = (source + 1) / 2
                    loss_illum = self.criterion_illum(source_01, illum, reflect)
            ...
            epoch_losses['illum'] += loss_illum.item()
            ...
            pbar.set_postfix({
                'Loss': f"{loss.item():.4f}",
                'Char': f"{loss_charbonnier.item():.4f}",
    ...
    def save_samples(self, epoch: int, num_samples: int = 4):
        ...
        # Also get illumination map for visualization
        illum = self.model.get_illumination(source)
        ...
        def illum_to_image(tensor):
            """Convert [0, 1] illumination to grayscale PIL Image."""
            tensor = tensor.squeeze(0).clamp(0, 1)
            tensor = tensor.cpu().numpy()
            tensor = (tensor * 255).astype(np.uint8)
            return Image.fromarray(tensor, mode='L').convert('RGB')

        # Create comparison grid: source | output | target | illumination
        for i in range(actual_samples):
            src_img = tensor_to_image(source[i])
            out_img = tensor_to_image(output[i])
            tar_img = tensor_to_image(target[i])
            ill_img = illum_to_image(illum[i])
            ...
            comparison.paste(ill_img, (width * 3, 0))
        ...
        if is_best:
            ...
            if self.ema is not None:
                ...
                torch.save(
                    ...
                    self.checkpoint_dir / "best_model_ema.pt"
                )
                self.ema.restore()
            else:
                torch.save(
    ...
    def train(self):
        """Main training loop."""
        print(f"\nStarting RetinexMamba training from epoch {self.start_epoch}")
        print(f"Training samples: {len(self.train_loader.dataset)}")
        print(f"Validation samples: {len(self.val_loader.dataset)}")
        print(f"Batch size: {self.batch_size}")
        print(f"Image size: {self.image_size}")
        print(f"Model: RetinexMamba-{self.model_size}")
...
def main():
    parser = argparse.ArgumentParser(description="Train RetinexMamba for HDR Enhancement")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs_retinexmamba",
                        help="Output directory")

    # Model arguments
    parser.add_argument("--model_size", type=str, default="base",
                        choices=["small", "base", "large"],
                        help="Model size: small (~5M), base (~12M), large (~25M)")
    ...
    parser.add_argument("--lambda_illum", type=float, default=0.01)
